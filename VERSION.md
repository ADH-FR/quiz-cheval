...
**`action: get_profile`**
1. Convertit `answers[]` en texte lisible (ex: "Q1 → Je réfléchis trop à tout")
2. Envoie à Claude Haiku via `https://api.anthropic.com/v1/messages`
3. Retourne un JSON : `{ profil, emoji, sous_titre, pourcentage, description, conseil_cle, profil_secondaire }`

**`action: save_result`**
1. Insère dans la table Supabase `quiz_resultats` : réponses, profil, description IA, timestamp
